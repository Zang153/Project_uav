...
def main():
    ...
    save_path = os.path.join(project_dir, "rl_results", "models")
    log_path = os.path.join(project_dir, "rl_results", "logs")
    ...
    # 将 config 中的时长转换为步数（假设控制频率为 100Hz）
    ...
    # 为了保证训练充分，增加训练时长
    total_train_steps = int(RL_TOTAL_TRAIN_SEC * 2 * control_freq)
    ...
    # 6. 保存最终模型
    final_model_path = os.path.join(save_path, "ppo_hover_final")
