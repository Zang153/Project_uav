...
# 跨平台兼容处理：macOS下运行MuJoCo图形界面时，Matplotlib必须使用非交互式后端
# 否则会产生主线程冲突导致崩溃
if platform.system() == 'Darwin':
    matplotlib.use('Agg') 
    print("检测到 macOS 系统，已启用 Matplotlib Agg 后端（图像将保存为文件而不是弹窗显示）")
else:
    # Linux/Windows 下通常可以正常显示，但也可以选择默认
    pass
    ...
    def plot_results(self):
        ...
        try:
            ...
            if platform.system() == 'Darwin':
                ...
                print(f"结果图表已保存为 {output_file} (macOS 兼容模式)")
            else:
                # Linux/Windows 下尝试直接显示
                try:
                    plt.show()
                except Exception as e:
                    # 如果显示失败（例如在无头服务器上），回退到保存图片
                    print(f"无法显示图形 ({e})，正在保存为文件...")
                    plt.savefig('simulation_results.png')
                    print("结果图表已保存为 simulation_results.png")
...
if __name__ == "__main__":
    ...
    xml_path = os.path.join(current_dir, "meshes/Delta.xml")
    simulator = OptimizedDroneSimulation(xml_path)  # 
    ...
    simulator.run_simulation(duration=20, target_trajectory=trajectory3)
